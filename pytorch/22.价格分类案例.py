...
def create_dataset():
    # 1. 加载csv文件数据集.
    data = pd.read_csv('pytorch/data/手机价格预测.csv')
...
def train(train_dataset, input_dim, output_dim):
    ...
    torch.save(model.state_dict(), 'pytorch/model/phone.pth') # 后缀名用: pth, pkl, pickle均可.


# 测试
def evaluate(test_data,input,output):
#创建神经网络模型
    model = PhonePriceModel(input,output)
#加载训练好的模型参数
    model.load_state_dict(torch.load('pytorch/model/phone.pth'))
#创建数据加载器
    test_loader = DataLoader(test_data,6,shuffle=False)
#定义变量，记录预测正确的样本个数
    correct=0
...
#预测模型
    ...
    print(f'acc{correct/len(test_data):.4f}')
